...
// echo of what it just created.
//
// Creating a sprint is one of the only calls in TAM that reach Jira outside
// a Commit, alongside StartSprint and CompleteSprint above: a sprint's id
// has to be real before anything else, a board column, an issue move,
// another edit, can point at it, and TAM has the machinery to defer an
// issue's id behind a Commit and none to defer a sprint's.
...
//
// Editing a sprint is one of the only calls in TAM that reach Jira outside
// a Commit, for the same reason CreateSprint's comment above gives: a
// sprint's identity is a shared fact the moment it exists, not something
// this app can hold back for a Commit to push later.
...
//
// Deleting a sprint is one of the only calls in TAM that reach Jira outside
// a Commit, for the same reason CreateSprint's comment above gives.
