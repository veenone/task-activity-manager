  ...
  return (
    <div className="profile-form">
      ...
      <details className="profile-form-advanced">
        ...
        {isEdit && ritualPages.length > 0 && <div className="ritual-association-form"><span className="field-hint">Associate a child page</span><select aria-label="Ritual page" value={ritualPageID} onChange={(event) => setRitualPageID(event.target.value)}><option value="">Choose a page…</option>{ritualPages.filter((page) => !ritualAssociations.some((association) => association.pageID === page.id)).map((page) => <option key={page.id} value={page.id}>{page.title}</option>)}</select><select aria-label="Ritual type" value={ritualType} onChange={(event) => setRitualType(event.target.value)}>{["planning", "standup", "review", "retro"].map((type) => <option key={type} value={type}>{type}</option>)}</select><button type="button" className="btn btn-ghost" disabled={!ritualPageID} onClick={() => { const page = ritualPages.find((item) => item.id === ritualPageID); if (!page) return; void SetRitualAssociation(profile!.id, { boardID: 0, sprintID: 0, ritualType, pageID: page.id, pageTitle: page.title }).then(() => ListRitualAssociations(profile!.id, 0, 0)).then((items) => { setRitualAssociations(items); setRitualPageID(""); }); }}>Associate</button></div>}
      </details>
