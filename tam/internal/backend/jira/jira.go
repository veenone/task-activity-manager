...
// SearchIssuesPage runs one page of the scope JQL and maps the rows. The
// scope is every type the project has except Xray's, so an issue of a type
// TAM does not model comes back under the project's own name for it.
func (b *Backend) SearchIssuesPage(ctx context.Context, projectKey, scopeJQL, since string, startAt, maxResults int) ([]backend.Issue, int, error) {
	ids := b.discover(ctx)
	pt := b.typesOrEmpty(ctx, projectKey)
	jql := buildJQL(projectKey, scopeJQL, since, pt.xray)
