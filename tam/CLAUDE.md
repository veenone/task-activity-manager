...
Outline mirrors this file for readers who are not in the repository,
under **Tools -> Task Activity Manager (TAM)**: a User Guide (end users),
Feature List, Supported Views, Architecture, Code Structure, Developer
User Guide, and Change Log, alongside XTM's own pages in the same
collection. Keep those pages in step when a change here alters what the
app does; the User Guide is the one written for people who will never
read this file.
