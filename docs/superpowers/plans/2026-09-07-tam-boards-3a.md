...
9. **A draft card sits in the first column, and it gets there on its own.** A draft has no Jira status (`writes.go` inserts `StatusDraft`) and no status id, so it matches no column. Two consequences the first draft of this plan got wrong. First, the test for a draft is `Issue.Draft`, which `scanIssue` sets from the key prefix, never "the status id is empty": right after the version 4 migration every cached issue has an empty status id, and matching on that would pile an entire backlog into the first column. Second, Jira's board issue list can never name a `TAM-NEW-n` key, so a draft has no way onto the board through membership; `IssueSource` grows a `DraftIssues` read and the view adds them itself. Without that the rule has no producer and the feature this board exists for is dead code.
...
`IssueSource` is the small interface `boardrepo` needs so it never imports `issuerepo`: `IssuesByKeys(ctx, profileID string, keys []string) ([]backend.Issue, error)` and `DraftIssues(ctx, profileID string) ([]backend.Issue, error)`. `app.go` passes the repository, and `app.go` is also where `var _ boardrepo.IssueSource = (*issuerepo.Repository)(nil)` belongs, since it is the one place that imports both. Swimlane values are `none`, `assignee`, `epic`; anything else is an error naming the three. Lanes keep the card order the `board_issue` positions give, and lanes themselves sort by label with the empty value last under "Unassigned" or "No epic". `ColumnView` carries `Total` and `Points` summed over the lane cells of that column.
...
- **A draft lands in the first column that has at least one status id**, not in `Unmapped`. It is the reason this board exists, and hiding it would be the worst outcome of the phase. Two qualifiers, both load-bearing. A board's first column is often a Backlog column with no statuses mapped to it, and dropping every draft into a column Jira itself never fills would be its own kind of wrong; a board with no column that has statuses has nowhere to put a draft, so it counts as unmapped there. And the test is `Issue.Draft`, never an empty status id: a cached issue synced before version 4 also has an empty status id, and treating those as drafts would draw the whole backlog in column one.
- **The drafts come from `IssueSource.DraftIssues(ctx, profileID)`, not from board membership.** Jira's board issue list cannot name a key Jira has never seen, so a draft reaches the board only because the view fetches it. Drafts are project-level and appear on every board and every sprint of that profile, wearing the Draft chip that says why they are there. They count into the column totals and obey both caps like any other card.
...
Every token above exists in `frontend/core/styles/tokens.css` and was checked against it: `--surface`, `--surface-3`, `--row-hover`, `--border`, `--text-muted`, `--accent`. There is no `--surface-1` in this design system (the scale is `--surface`, `--surface-2`, `--surface-3`, `--surface-sunken`); do not invent one. The sticky column head needs an opaque background so the cards cannot show through it as they scroll under, and it takes `--surface-2`, which is what the mockup's `#F3F4F6` header is closest to; `--surface-3` is so near white that the head would stop reading as a head.
...
Under the toolbar, the line that orients a standup, because orientation is a read-only board's whole job and the sprint is the frame the conversation sits in: `.board-summary` reads "Sprint 12, active, ends 12 Sep. 27 of 47 points done." from the selected sprint's `startDate` and `endDate`, the summed `ColumnView.Points` for the total, and, for the done half, the points of the cards whose status is done by `src/lib/statusClass.ts`, which is the definition the grid and the epic tree already use. Not the rightmost column: a board's last column is as often Blocked or Won't Do as it is Done, and the number this line reports is the one a standup actually argues about, then "synced 12 min ago" from `useSyncState(profileId).lastSynced`. Format the age with `formatWhen` from `src/lib/format.ts`, which the status bar already uses; a second time helper in the same window would give one fact two different wordings.
...
| `needsStatusSync` | "Sync to draw this board", with the Sync button as the next step, **and no grid at all**. Right after the version 4 upgrade no cached issue has a status id yet, so every card would be unmapped; a board that drew an empty grid and blamed the user for it would be the worst first impression this feature could make |
