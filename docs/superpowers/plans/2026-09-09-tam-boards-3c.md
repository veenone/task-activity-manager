...
- A lifecycle action is refused before it is attempted when the app has no connection, with the same sentence the disabled button carries. It never journals a fallback.
...
2. **The end date is guessed from the board's own history.** The median length of the last three closed sprints is right far more often than an empty field, and a board with no history gets two weeks with the field focused.
...
**Produces:** `(*Client).CreateSprint(ctx, boardID int, name, goal, start, end string) (RawSprint, error)`, `(*Client).StartSprint(ctx, sprintID int, name, goal, start, end string) error`, `(*Client).CompleteSprint(ctx, sprintID int) error`; on `BoardBackend`: `CreateSprint(ctx, boardID int, s backend.SprintDraft) (backend.Sprint, error)`, `StartSprint(ctx, sprintID int, s backend.SprintDraft) error`, `CompleteSprint(ctx, sprintID int) error`; `backend.SprintDraft{Name, Goal, StartDate, EndDate string}`.

- [ ] **Step 1: The tests.** Extend the httptest server in `agile_test.go` with `POST /rest/agile/1.0/sprint` and `POST /rest/agile/1.0/sprint/12`. Cover: a create sending name, `originBoardId`, goal and both dates, and decoding the sprint Jira returns; a start sending `state: "active"` with the dates; a completion sending `state: "closed"` **and nothing else**, since a completion that also sends dates would rewrite them; a 400 whose Jira message survives into the error, which is how "another sprint is already active on this board" reaches the user; and a 403 likewise. Assert on the request bodies, not only the status: these are all side effect.
...
- [ ] **Step 2: The service.** `internal/sprints` owns the two lifecycle actions so the online-only rule has one home and one place to test. `Start` calls the backend and returns Jira's error unchanged. `Complete` reads the sprint's incomplete issues from the cache, moves them with `MoveIssuesToSprint` to the backlog or the named sprint **first**, closes the sprint **second**, and returns a `Completion{Moved int, MovedTo string}` for the view to report. A failed move returns before the close, with the sprint still open and an error saying so. Neither method touches the journal.

- [ ] **Step 3: The bindings.** `StartSprint` and `CompleteSprint` run under `a.acquire(p.ID, "sprint")`, which is a real guard here because they push to Jira, unlike the journal writes 3b deliberately left unguarded. Both end with a boards sync so the picker and the board match Jira. `MoveIssuesToSprint` is the bulk journal write: it loops `issuerepo.MoveToSprint` inside one transaction and is not guarded, matching every other journal write. `SuggestSprintDates` returns today, today plus `SprintLength` days (two weeks when there is no history), and the name the board's last sprint suggests.
...
- [ ] **Step 2: The board honours it.** A selected card takes `.board-card-selected` (which already exists) and `aria-selected`; the count appears in the toolbar as "3 selected" with a Move to sprint action beside it and a way to clear. Selection clears on any change of board, sprint, swimlane or profile, through the same render-time reset the view already uses for its filters. Keyboard: Shift with the arrows extends, Control with Space toggles, matching what the grid roles already promise.

- [ ] **Step 3: The start dialog.** `StartSprintModal` opens from the toolbar for a future sprint: name prefilled, goal, start date defaulting to today and end date to today plus the suggestion, both editable, with the field focused when there was no history to suggest from. Submit calls `StartSprint` and reports Jira's message on failure, in the dialog, without closing it.

- [ ] **Step 4: The complete dialog.** `CompleteSprintModal` opens for the active sprint. It names how many issues are incomplete, from the board's own cards, and offers where they go: the backlog, or any future sprint of that board. Submit calls `CompleteSprint` and reports what moved and where, or leaves the sprint open with the error when the move failed.

- [ ] **Step 5: Offline.** Both buttons are disabled with the sentence the spec names when the profile has no connection, and the same sentence is what the binding returns if one is somehow attempted. A demo profile is never offline.

- [ ] **Step 6: The detail panel.** The Details tab gains Sprint as an editable field: a select of the board's open sprints plus the backlog, going through the same `MoveIssueToSprint` binding the card menu uses, so the pending row, the discard and the commit path are the ones 3b already reviewed. It is the last thing Decision 6 of the 3b design promised and did not ship.

- [ ] **Step 7: Tests.** `boardSelection.test.ts` for the three gestures including a shift-click that crosses the anchor. `BoardsView.test.tsx`: three cards selected shows the count, the bulk move calls the binding once with three keys, a sprint change clears the selection, the start dialog's dates default from the suggestion, the complete dialog names the incomplete count and its destinations, a failed start keeps the dialog open with Jira's message, and both buttons are disabled offline. `IssueDetailPanel.test.tsx`: the sprint select journals through the same binding.

- [ ] **Step 8: Commit** as `feat(tam): start a sprint, complete it, and plan into it`.
...
Creating a board, editing a sprint's dates once it has started, a bulk transition and a bulk rank (both need every card's workflow checked, which is its own design), sprint reports and burndown (Phase 4, which is what these completed sprints feed), and the board's own quick filters and swimlane rules.
