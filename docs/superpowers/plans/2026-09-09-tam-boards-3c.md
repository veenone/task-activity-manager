...
2. **The end date is a suggestion that says where it came from.** Jira's Agile API does not expose a board's configured sprint length: the board configuration endpoint carries columns and estimation, not cadence. So the only honest sources are the board's own history or a flat default, and the median of the last three closed sprints is the better of the two. The objection to it stands, though, and is answered in the dialog rather than in the arithmetic: a team that changed cadence, or a quarter with a holiday sprint in it, produces a plausible wrong date nobody checks, so the field says whether it came from the last sprints or from the two week default, and it is editable either way. An earlier draft of this decision named the board configuration as the source, which cannot be read.
...
- [ ] **Step 1: The tests.** Extend the httptest server in `agile_test.go` with `POST /rest/agile/1.0/sprint/12`. Cover: a start sending `state: "active"` with the dates; a completion sending `state: "closed"` **and nothing else**, since a completion that also sends dates would rewrite them; a 400 whose Jira message survives into the error, which is how "another sprint is already active on this board" reaches the user; and a 403 likewise.

The message does not survive today: `writeStatusError` formats a failed write with `bodyExcerpt`, a raw slice of the body, where `Get` uses `jiraErrorMessage` and gets Jira's sentence. So the user would read a fragment of JSON where the spec promises a sentence naming the active sprint. Fix it in the client rather than in each caller, and keep the excerpt as the fallback for a body that is not Jira's error shape. Assert on the request bodies, not only the status: these are all side effect.
...
Name the date format in those doc comments and convert to it in one place. The Agile API wants `2026-09-09T09:00:00.000+0000` and rejects a bare `2026-09-09`, and an HTML date input produces exactly the bare form, so the conversion belongs in `sprints.Service` where both dates pass through, not in the dialog and not in the client. Parse with `time.Parse` and refuse what does not parse, rather than concatenating a suffix onto whatever arrived; use the machine's own offset rather than a hardcoded `+0000`, which would shift a non-UTC team's burndown by a working day. The end-before-start check lives here too, not only in the dialog, because the bound method is reachable without it.
...
- [ ] **Step 1: Read in a transaction.** Give `Repository` a small `inReadTx` helper beside the existing `inTx`, opening a deferred transaction, running the reads, and rolling back (a read transaction has nothing to commit). `Board` and `CellOrder` take all of their statements through it, including the ones behind `IssueSource`, which means `IssuesByKeys`, `DraftIssues` and `PendingMoves` need a form that accepts the transaction. This is a new pattern, not an existing one, and the plan says so rather than sending an implementer to look for it: every transaction-scoped query in `boardrepo` is a private helper on `*sql.Tx`, and the three reads in question live in `issuerepo` behind the exported `IssueSource` interface, which takes no transaction. Closing this means `IssueSource` grows a querier parameter, which changes an exported interface, the wiring in `app.go`, and every fake in `view_test.go`. Budget that; it is the real cost of this task.
...
- [ ] **Step 2: The service.** `internal/sprints` owns the two lifecycle actions so the online-only rule has one home and one place to test. `Start` calls the backend and returns Jira's error unchanged. `Complete(ctx, profileID string, boardID, sprintID int, moveTo string)` takes the board, which an earlier draft omitted while requiring a read that needs it. It re-reads the sprint's issues **from Jira**, not from the cache, which can be minutes stale and would otherwise decide the fate of cards on data the user cannot see. It cannot do that with `BoardIssueKeys`, which returns keys and no statuses, and "incomplete" is defined by status: it uses the issue search the syncer already has, one paged `sprint = N` query, which brings back the status with the key in one call rather than one call per card. It pushes the incomplete ones to the backlog or the named sprint with the backend's `PushIssuesToSprint` **first**, in chunks of twenty for the same reason the bulk journal move chunks, closes the sprint **second**, and returns `Completion{Moved int, MovedTo string, Failed []string}` **together with the error**, not instead of it. A chunk that fails partway leaves the earlier chunks already out of the sprint while the local membership still says they are in it, so the service refreshes that board's membership before returning: a user who cancels at that point must not be left with cards that vanished from an open sprint with nothing recording where they went. A completion of forty issues that loses one chunk must be able to say "twelve of forty moved, the sprint is still open", which a count alone cannot express.
...
- [ ] **Step 3: The bindings.** `StartSprint` and `CompleteSprint` run under `a.acquire(p.ID, "sprint")`, which is a real guard here because they push to Jira, unlike the journal writes 3b deliberately left unguarded. Both end by refreshing **that board's sprints only**, one `Sprints` call and an upsert through a new exported `boardrepo.ReplaceSprints(ctx, profileID string, boardID int, sprints []backend.Sprint) error`, which does not exist yet: `writeSprints` is private and only reachable through `ReplaceBoard`, which would delete and rewrite the board's columns and every membership row on its way past. Calling that to refresh a sprint list would wipe the board. Not a boards sync either: `tam/CLAUDE.md` records that the boards pass takes minutes on a real project and acquires under its own name, so ending a lifecycle action with one would block the UI for minutes or be refused outright by the lock the action itself is holding. The same file's rule applies here too, that anything calling a bound method which takes `acquire` goes through the sync reducer, so the two lifecycle actions run through `SyncContext` the way `runBoardsRefresh` already does. `JournalSprintMoves` is the bulk journal write, and it cannot be written the obvious way: `issuerepo.MoveToSprint` already opens its own transaction through `inTx`, so looping it inside an outer one takes a second pooled connection, hits `SQLITE_BUSY` the moment the outer transaction has written, and burns the five second `busy_timeout` per key. Twenty keys would freeze the UI for a hundred seconds and then fail. Extract the body into an unexported `moveToSprintTx(ctx, tx, ...)` and have both the single and the bulk path call it, which is the shape the package already uses elsewhere. It is not guarded, matching every other journal write, and it keeps the `strconv.Atoi` guard and the sprint name argument the single binding already has.

Two behaviours the bulk path inherits and must be tested for rather than assumed: a card already in the target sprint journals nothing (so "one row per key" is wrong for any realistic selection), and an uncached key returns `ErrNotFound`, which must not abort the other nineteen. `SuggestSprintDates` returns today, today plus `SprintLength` days (two weeks when there is no history), the name the board's last sprint suggests, and whether the length came from that history or from the default, so the dialog can say which.

- [ ] **Step 4: Tests, then the one Go suite run.** Beyond the service's own cases, four the review named because each is a Friday night: a `Board` read racing `ReplaceBoard`, which is the only test that proves Task 2; a bulk move where one key is uncached and another is already in the target sprint; a completion whose second chunk of three fails, asserting the sprint is still open and asserting what the cache says afterwards; and a lifecycle call refused while a boards refresh holds the lock, with the refusal naming which operation is running. `sprints_test.go` with a fake backend: a start that passes the draft through; a start Jira refuses, whose message survives; a completion that moves three issues to the backlog then closes; a completion whose move fails, leaving the sprint open and reporting it; a completion with no incomplete issues that closes without a move. `app_boards_test.go`: the guard refuses a second lifecycle call while one runs, and a bulk move journals one row per key. Then, inside `tam/`: `go build ./... && go vet ./... && go test ./... -count=1`, and inside `core/`: `go test ./jira/ -count=1`. Fix what fails, rerun at most twice, report. Commit as `feat(tam): the sprint lifecycle, its dates, and a bulk move`.
...
It also has to answer for the user's own uncommitted work, which nothing else in this app has had to do: a card the user dragged to Done an hour ago is Done on their screen and not in Jira, and closing the sprint would move it to the backlog as incomplete. When any pending journal row targets an issue in this sprint, the user is stopped **before the dialog opens**, not after they have chosen a destination: the toolbar's Complete button asks first, through a `useConfirm` naming Commit as the thing to do. That needs a binding, because pending rows carry issue keys and know nothing about sprints, so Task 3 also produces `PendingInSprint(profileID string, sprintID int) (int, error)`, living in `app_boards.go` since it spans both repositories. It counts rows whose issue is in the sprint **and is staying there**: a pending move *out* of the sprint is not a reason to stop, and counting it would block a completion for the one case the user has already resolved. Board writes are deliberately unguarded, so a drag can land between the check and Submit; `sprints.Service.Complete` re-checks and refuses rather than trusting the toolbar's answer.
...
- **A completion only sees the issue types TAM syncs.** The sprint's own read is the issue search, scoped by `backend.AllTypes`, which is the same five types the sync scope names: task, epic, story, bug and the profile's requirement type. A project that defines a type of its own puts cards in the sprint that the completion never counts, never moves to the chosen destination, and never names in `Failed`. Jira's own close behaviour then sends them to the backlog, which is where an unfinished card goes when nobody said otherwise, so nothing is lost and nothing is silently resolved, but the destination the user chose did not apply to them. `sprints.Completion` says so on the type rather than presenting `Moved` and `Failed` as the size of the sprint, and the dialog's counts are counts of what the completion considered. Reading the sprint by its own membership instead needs a call that returns statuses with the keys, which is the reason the search was chosen in the first place.
- **A single 400 on the sprint endpoint reads as "this board has no sprints".** `core/jira/agile.go` maps any 400 on the first page to `ErrNoSprints`, because that is how a kanban board answers, and the Jira backend turns that into an empty slice and no error. The ceremonies no longer write an empty list back over a board they have just acted on, so the picker and the sprint length survive it, but a board that genuinely lost its last sprint keeps the stale row until the next boards sync. Telling the two apart properly means reading the 400's body, which belongs with whoever next has a real instance to read one from, beside the 207 above.

## Deferred

Creating a board, editing a sprint's dates once it has started, and a bulk transition, a bulk rank, a bulk assignee and bulk points. That last group is a cut rather than a completion: selecting several cards and being offered one action reads as unfinished, so the toolbar action is worded as exactly what it does, "Move N cards", with the destination picker beside it naming the backlog or any of the board's other open sprints, and promises nothing else. A bulk transition needs every selected card's workflow checked, which is its own design, sprint reports and burndown (Phase 4, which is what these completed sprints feed), and the board's own quick filters and swimlane rules.
