...
**Produces:** `(*Client).CreateSprint(ctx, boardID int, name, goal, start, end string) (RawSprint, error)`, `(*Client).StartSprint(ctx, sprintID int, name, goal, start, end string) error`, `(*Client).CompleteSprint(ctx, sprintID int) error`; on `BoardBackend`: `CreateSprint(ctx, boardID int, s backend.SprintDraft) (backend.Sprint, error)`, `StartSprint(ctx, sprintID int, s backend.SprintDraft) error`, `CompleteSprint(ctx, sprintID int) error`; `PushIssuesToSprint` is 3b's existing `MoveIssuesToSprint` on the same interface, referred to by that name throughout this plan so it is never confused with the journal binding; `backend.SprintDraft{Name, Goal, StartDate, EndDate string}`.
...
- [ ] **Step 4: The complete dialog.** `CompleteSprintModal` opens for the active sprint. It names how many issues are incomplete and offers where they go: the backlog, or any future sprint of that board. Submit calls `CompleteSprint` and reports what moved and where, or leaves the sprint open with the error when the push failed.

It also has to answer for the user's own uncommitted work, which nothing else in this app has had to do: a card the user dragged to Done an hour ago is Done on their screen and not in Jira, and closing the sprint would move it to the backlog as incomplete. When any pending journal row targets an issue in this sprint, the dialog says how many and requires an explicit confirmation, naming Commit as the thing to do first. That is a blocking confirm, not a warning line.
