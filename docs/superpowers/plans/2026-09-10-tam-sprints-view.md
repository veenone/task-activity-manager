...
**Files:** create `SprintsView.tsx`, `SprintList.tsx`, `SprintRow.tsx`, `EditSprintModal.tsx`, `SprintFillBar.tsx`, `ImmediateWriteChip.tsx`, `lib/sprintGroups.ts`, `lib/unfinished.ts`, `queries/sprints.ts`, and a test file for each; modify `SprintDraftForm.tsx`, `CreateSprintModal.tsx`, `frontend/core/src/components/useConfirm.tsx`, `queries/keys.ts`, `lib/format.ts`, `nav.ts`, `App.tsx`, `main.go`, `BoardCeremonies.tsx`, `StartSprintModal.tsx`, `CompleteSprintModal.tsx`, `EpicRow.tsx`, `App.css`, **and the test files of every shipped component this touches**, which are not otherwise in this list and are what Task 9's gate run would otherwise fail on.
...
- [ ] **Step 7: The dialogs and the marker.** `SprintDraftForm` already exists: it was lifted out of `StartSprintModal` during part one, when the create dialog had made a second copy of it and this task would have made a third. `CreateSprintModal` already renders it too. What is left here is the Edit dialog, and one thing neither existing dialog does: **seed the goal from the sprint**, now that the goal is in the cache. Without it the Start dialog opens an empty goal box over a real goal and the user types over it blind, which is the same blindness this branch added the column to remove. A duplicate name nudges without blocking. Both dialogs disable on submit. Edit sends `clearGoal` and confirms before moving a running sprint's end date. `ImmediateWriteChip` is `chip chip-now` reading "Sends to Jira now", painted accent, **never amber**, in four dialogs' `.pending-head` and the confirmation, never on a menu item; `CompleteSprintModal` keeps its irreversibility banner, which is a different claim. `ConfirmOptions.message` widens to `ReactNode`.
