...
**Decisions**
1. Entities `sprint_start` and `sprint_complete` in `issuerepo` beside `sprint_edit` and `sprint_delete`, one row per sprint keyed by the sprint id as text. `sprint_start` carries `{boardId, name, goal, startDate, endDate}` with the dates already converted; `sprint_complete` carries `{boardId, name, moveTo, moveToName, previewCount}`, `moveTo` empty for the backlog.
2. Neither changes the cached sprint state. The Sprints view shows "Starting on Commit" or "Completing on Commit" through the chip Task 7 added for a pending delete; Pending changes reads "Start sprint X, <start> to <end>" and "Complete sprint X, unfinished cards to <Y or the backlog>", each with a Discard that only drops the row.
3. Refused locally with nothing written: a start unless the cached state is `future`, or while a delete is pending; a completion of a future or draft sprint (the `requireCompletable` wording), into a draft or into itself (`destinationID`), while cards in the sprint have pending changes (`refusePending`), or while a delete is pending. A delete is now also refused while a start or a completion is pending.
4. A draft sprint can be started in the same Commit: `sprint_start` is allowed on a negative id, `RekeySprint` moves the row to the real id and the real board, and discarding the draft drops it. The committer's sprint work splits into two phases, `sprints` (creates) and `sprint changes` (edits, then starts, then completes, then deletes), so the second reads the journal the creates rewrote. A start still keyed by a draft id is held when the create was blocked, and fails for good when its create row is gone.
5. `Start` and `Complete` move onto `sprints.Committed`; `Service` exports no write method, and `exceptions_test.go` is deleted. The completion recomputes the unfinished set from Jira at push time, and `Result.SprintsChanged` reports the real count, "Sprint 12 completed, 45 unfinished cards moved to Sprint 13". A completion that stopped after moving cards is a retryable failure naming the moved keys, and the row stays; a retry against a sprint the cache already holds as closed moves nothing and succeeds.
6. `StartSprint` and `CompleteSprint` need no backend and work offline; `CompleteSprint` takes the dialog's preview count and answers only an error. The Complete dialog says "About N cards are not finished", N from the cached board, and that Commit works the set out again; neither dialog carries the "Sends to Jira now" chip, which is deleted. Start is offered on a draft sprint.
