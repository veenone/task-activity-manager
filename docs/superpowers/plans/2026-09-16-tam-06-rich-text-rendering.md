...
5. **Entities are decoded in ordinary text only.** ~~marked's lexer returns escaped text~~ — corrected on 2026-09-16 after measuring `marked@18.0.13`: its `lexer()` does not pre-escape. CommonMark (and marked's own renderer) decodes entity references everywhere except code spans and code blocks, so `markdown.ts` decodes `&amp;` `&lt;` `&gt;` `&quot;` `&#39;` in `text` and `escape` tokens and leaves `codespan` and the raw-HTML fallback byte for byte. A description that quotes `&amp;amp;` inside backticks must keep it.
